...
- Tentativas de acesso negado são registradas em logs estruturados para auditoria`,
    ...
    tags: [
      ...
      { name: 'Admin Users', description: 'Gestão de usuários do sistema (apenas admin)' },
      { name: 'Admin Plans', description: 'Configuração de planos pelo admin' },
      { name: 'Admin Referrals', description: 'Gestão de referências/indicações (apenas admin)' },
      { name: 'Admin TimeBlocks', description: 'Bloqueios de horário na agenda' },
      ...
      schemas: {
        ...
        AdminUserItem: {
          ...
          properties: {
            ...
            professionalsTotal: { type: 'integer' },
            createdAt: { type: 'string', format: 'date-time' },
            trialDaysRemaining: { type: 'integer', nullable: true, description: 'Dias restantes de trial (null se plano pago)' },
            hasPaidPlan: { type: 'boolean' },
            plan: {
              type: 'object',
              properties: {
                planId: { type: 'string', enum: ['FREE', 'PREMIUM', 'PRO', 'ADMIN'] },
        ...
        Referral: {
          ...
          properties: {
            ...
            commissionValue: { type: 'integer', description: 'Centavos para fixed, inteiro para percentage' },
            createdAt: { type: 'string', format: 'date-time' },
            updatedAt: { type: 'string', format: 'date-time' },
          },
        },
